...
const keyBytes = Uint8Array.from(JSON.parse(readFileSync('/home/obrera/keys/obrE1BHvP4EX8PkxPxAJxYfQkgfgCmXyJadQA3yBb7G.json','utf8')));
...
const nonce = await api('/api/auth/solana-auth/nonce', { method: 'POST', body: JSON.stringify({ walletAddress: wallet }) });
const message = [
...
await api('/api/auth/solana-auth/verify', { method: 'POST', body: JSON.stringify({ walletAddress: wallet, message, signature: toBase58(signature) }) });
const bootstrap = await api('/api/bootstrap');
console.log(JSON.stringify(bootstrap.entitlements, null, 2));
